...
const typeColors: Record<EventType, string> = { attended: "from-teal-500 to-teal-400", organized: "from-cyan-500 to-cyan-400", mentored: "from-teal-400 to-cyan-500", "organized+attended": "from-teal-500 to-cyan-500" };
      ...
      <div className="relative overflow-hidden min-h-screen sm:max-w-[50%] mx-auto sm:border sm:border-teal-500 rounded-xl p-6">
        <div className="mb-16 relative">
          ...
          <div className="relative">
            <motion.div className="absolute -top-6 left-0" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 0.5 }}><Star className="w-5 h-5 text-cyan-400/30" /></motion.div>
            <motion.div className="absolute -top-2 left-20" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 0.7 }}><Star className="w-3 h-3 text-teal-400/40" /></motion.div>
            <motion.div className="absolute top-10 right-10" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 0.9 }}><Star className="w-4 h-4 text-cyan-400/30" /></motion.div>
            <motion.div className="relative z-10" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              <div className="mb-16 text-center">
                <h1 className="text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-teal-400 via-cyan-400 to-teal-400 mt-5 mb-2 py-5">Professional Journey</h1>
                <motion.div className="h-1 w-32 mx-auto bg-gradient-to-r from-teal-500 to-cyan-500 rounded-full mb-4" initial={{ width: 0 }} animate={{ width: 128 }} transition={{ duration: 0.8, delay: 0.3 }} />
                <p className="text-t-muted max-w-2xl mx-auto">A showcase of major events I have attended, hackathons I have organized, and programs where I have served as a mentor. Each experience has shaped my professional growth in the tech industry.</p>
              </div>
            </motion.div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-8">
          {[...events].sort((a, b) => b.id - a.id).map((event, index) => (
            <motion.div key={event.id} className="group relative bg-t-bg border border-teal-500/20 rounded-xl overflow-hidden transition-all duration-300" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }} whileHover={{ scale: 1.02, boxShadow: `0 0 25px 2px rgba(20, 184, 166, 0.3), 0 0 10px 0px rgba(6, 182, 212, 0.2)` }}>
              ...
              <div className="flex flex-col md:flex-row">
                <div className="md:w-1/3 p-6 flex items-center justify-center relative">
                  ...
                  <div className={`w-16 h-16 rounded-full bg-gradient-to-r ${typeColors[event.type]} flex items-center justify-center shadow-lg shadow-teal-500/20 group-hover:shadow-teal-500/40 transition-all duration-300 z-10`}>{typeIcons[event.type]}</div>
                </div>
                <div className="md:w-2/3 p-6">
                  <div className="flex items-center mb-3"><span className={`text-xs font-medium px-3 py-1 rounded-full bg-gradient-to-r ${typeColors[event.type]} text-white`}>{typeLabels[event.type]}</span></div>
                  <h3 className="text-xl font-bold text-t-text mb-3 group-hover:text-cyan-400 transition-colors duration-300">{event.title}</h3>
                  <div className="flex flex-wrap gap-4 mb-4">
                    <div className="flex items-center text-t-muted"><Calendar className="w-4 h-4 mr-2 text-teal-500" /><span className="text-sm">{event.date}</span></div>
                    <div className="flex items-center text-t-muted"><MapPin className="w-4 h-4 mr-2 text-teal-500" /><span className="text-sm">{event.location}</span></div>
                  </div>
                  <p className="text-t-muted text-sm mb-4">{event.description}</p>
                </div>
              </div>
